...
- (void)viewDidLoad
{
     ...
     */
    UserProfile * userProfile = [WTTSingleton sharedManager].userprofile;
    NSLog(@"%@", userProfile.email);
    if (userProfile.userimg != nil) {
        [_profilePhoto setImage:userProfile.userimg];
    }
    if (userProfile.username != nil)
    {
        [_nameLabel setText: userProfile.username];
    }
    if (userProfile.email != nil) {
        [_emailLabel setText:userProfile.email];
    }
    if (userProfile.school != nil){
        [_schooLabel setText:userProfile.school];
    }
    if (userProfile.major != nil){
        [_majorLabel setText:userProfile.major];
    }
    userProfile = nil;
